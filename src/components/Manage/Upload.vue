          ...
          <div class="p-6">
            ...
            <div
              class="w-full px-10 py-20 rounded text-center cursor-pointer border border-dashed
                ...
                :class="{'text-indigo-100 bg-green-400 border-green-400 border-solid': is_dragover}"
                @drag.prevent.stop=""
  ...
  methods: {
    upload($event) {
      ...
      files.forEach((file) => {
        ...
        const storageRef = storage.ref(); // music-546e8.appspot.com
        const songsRef = storageRef.child(`songs/${file.name}`); // music-546e8.appspot.com/songs/example.mp3
